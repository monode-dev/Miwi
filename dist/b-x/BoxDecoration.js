...
import { Align, _FlexAlign } from "./BoxLayout";
...
export function computeBoxDecoration(sty) {
    const shadowDirection = (() => {
        const givenDirection = sty.shadowDirection ?? Align.bottomRight;
        return {
            x: givenDirection.alignX === _FlexAlign.start
                ? -1
                : givenDirection.alignX === _FlexAlign.center
                    ? 0
                    : 1,
            y: givenDirection.alignY === _FlexAlign.start
                ? 1
    ...
    return {
        ...
        boxShadow: exists(sty.shadowSize)
            ? `${sizeToCss(0.09 * sty.shadowSize * shadowDirection.x)} ${sizeToCss(-0.09 * sty.shadowSize * shadowDirection.y)} ${sizeToCss(0.4 * sty.shadowSize)} 0 #00000045`
            : undefined,
        zIndex: sty.zIndex,
    };
}
