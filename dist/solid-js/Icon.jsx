...
function toMdiName(str) {
    return str.split("-").map(ucFirst).join("");
}
export function Icon(props) {
    return (<Box width={props.size ?? 1} height={props.size ?? 1} textColor={props.color ?? `currentColor`} overflowY={$Overflow.crop} onClick={props.onClick}>
      ...
      <svg fill="currentColor" width="100%" height="100%" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d={mdijs[`mdi${toMdiName(props.icon)}`]}></path>
      </svg>
    </Box>);
}
